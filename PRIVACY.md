...
- The assembled prompt is copied to the system clipboard as a fallback.
...
Settings remain in Chrome storage until the user changes them, clears extension data, or uninstalls the extension. A pending prompt is transient and is removed after use or expiration. Clipboard content remains until overwritten by the user or another application.
...
- 自動入力できない場合に備え、完成したプロンプトをOSのクリップボードへコピーします。
...
設定は、ユーザーが変更、拡張データを消去、または拡張をアンインストールするまでChromeストレージに保存されます。処理待ちのプロンプトは使用後または有効期限後に削除されます。クリップボードの内容は、ユーザーまたは別のアプリケーションによって上書きされるまで残ります。
